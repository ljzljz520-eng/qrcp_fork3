...
func init() {
	...
	rootCmd.PersistentFlags().IntVar(&app.Flags.ChunkSize, "chunk-size", 0, "chunk size in MiB for the resumable chunked transfer (default 4)")
	rootCmd.PersistentFlags().StringVar(&app.Flags.TTL, "ttl", "", "time to live of the transfer session, e.g. 30m or 24h (default 24h)")
